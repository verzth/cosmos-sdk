...
```go
import "github.com/verzth/cosmos-sdk/tools/confix/cmd"
```
...
The `ConfixCommand` function builds the `config` root command and is defined in the `confixCmd` package (`github.com/verzth/cosmos-sdk/tools/confix/cmd`).
...
```bash
go install github.com/verzth/cosmos-sdk/tools/confix/cmd/confix@latest
```
