...
```go reference
https://github.com/verzth/cosmos-sdk/blob/v0.47.0-rc1/store/types/gas.go#L40-L51
```
...
The anteHandler is not implemented in the core Cosmos SDK but in a module. That said, most applications today use the default implementation defined in the [`auth` module](https://github.com/verzth/cosmos-sdk/tree/main/x/auth). Here is what the `anteHandler` is intended to do in a normal Cosmos SDK application:
